...
import { issueUserAccessToken } from '../../utils/issue-user-token.js';
 ...
 *         token:
 *           type: string
 *           description: JWT for API access; expires in 90 days
 *     ApiSuccessUsersList:
 *       type: object
 *       properties:
 *         success: { type: boolean, example: true }
 ...
 *           application/json:
 *             schema:
 *               $ref: '#/components/schemas/ApiSuccessLogin'
 *       400:
...
async function loginUser(req, res) {
    ...
    try {
        ...
        if (error.message === 'JWT_SECRET is not configured') {
            console.error('[users] login JWT misconfiguration');
            res.status(500).json({
                success: false,
 ...
 *   post:
 *     tags: [Users]
 *     summary: Deactivate multiple user
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
